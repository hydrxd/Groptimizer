...
import { Home, List, PlusCircle, Clipboard, LogOut, Users, HeartHandshake, LayoutDashboard } from "lucide-react";
        ...
        if (!auth.token) {
            ...
            if (auth.role) {
                switch (auth.role) {
                    case "admin":
                        links.push(
                            { name: "Dashboard", to: "/dashboard", icon: <LayoutDashboard size={20} className="text-gray-300" /> },
